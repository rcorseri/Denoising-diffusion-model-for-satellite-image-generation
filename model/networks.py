...
def define_CD(opt):
    ...
    for i in range(0, len(feat_scales)):
        ...
        A = torch.randn(1,dim,h,w).to(device)
        #B = torch.randn(1,dim,h,w).cuda()
        B = torch.randn(1,dim,h,w).to(device)
        f_A.append(A)
        f_B.append(B)
        f_A.append(A)
        f_B.append(B)
    ...
    flops, params = profile(copy.deepcopy(netCD).to(device), inputs=(F_A,F_B,), verbose=False)
    flops, params = clever_format([flops, params])
    #netGcopy = copy.deepcopy(netCD).cuda()
    netGcopy = copy.deepcopy(netCD).to(device)
    netGcopy.eval()
    with torch.no_grad():
        start = time.time()
        _ = netGcopy(F_A, F_B)
